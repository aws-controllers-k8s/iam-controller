...
func (rm *resourceManager) Get(
	...
	if len(seeds) == 0 {
		parent.Spec.InlinePolicies = nil
		return nil
	}
	...
	parent.Spec.InlinePolicies = found.ko.Spec.InlinePolicyMap
	return nil
}
...
func convertFromParent(parent *svcapitypes.Role) []resource {
	items := parent.Spec.InlinePolicies
	if len(items) == 0 {
		return nil
	}
	...
	for mapKey, mapValue := range items {
		...
		{
			k := mapKey
			ko.Spec.PolicyName = &k
		}
		ko.Spec.PolicyDocument = mapValue
