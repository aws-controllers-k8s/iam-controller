...
// by concatenating the batch collection field (Spec.Tags).
func mergeResources(items []*resource) *resource {
	...
	merged.Spec.Tags = nil
	for _, item := range items {
		merged.Spec.Tags = append(
			merged.Spec.Tags,
			item.ko.Spec.Tags...,
		)
	}
...
func (rm *resourceManager) Get(
	...
	parent.Spec.Tags = found.ko.Spec.Tags
	return nil
}
...
func convertFromParent(parent *svcapitypes.Role) []resource {
	...
	for _, item := range items {
		...
		ko := &svcapitypes.RoleTags{}
		ko.Spec.Tags = append(ko.Spec.Tags, item)
		ko.Spec.Key = item.Key
